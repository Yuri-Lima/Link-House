...
void loop() {
  ...
  if ( M.currentMillis - M.previousMillis >= M.interval) {
    ...
    if (!M.flag)   {
      digitalWrite(CARGA3, !digitalRead(CARGA3));
      Serial.print("\nLuz ");
      Serial.println(!digitalRead(CARGA3) == 0 ? "Desligada" : "Ligada");
      M.flag = 1;
    }
  }
  ...
  if (wifi_station_get_connect_status() != 5) {
    // Se a conexao estiver em modo AP ele vai servir uma pagina para o cliente colocar os dados da rede wifi
...
#endif
    if (req.charAt(1) == 0x31)digitalWrite(CARGA3, !digitalRead(CARGA3));//1
  }
...
void connecT(char *ssid, char *pswd) {
  ...
  count = 0;
  if (WiFi.status() == WL_CONNECTED) ESP.restart();//Resta de conectado
}
//=====================================================================
void setupWiFi() {
  Serial.println("WiFi");
