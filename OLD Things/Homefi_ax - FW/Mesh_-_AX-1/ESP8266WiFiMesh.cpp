...
#define SSID_PREFIX      		"Home"
#define SERVER_IP_ADDR			"192.168.4.1"
#define SERVER_PORT				4011

ESP8266WiFiMesh::ESP8266WiFiMesh(uint32_t chip_id, std::function<String(String)> handler): _server(SERVER_PORT) {
  _chip_id = chip_id;
...
bool ESP8266WiFiMesh::exchangeInfo(String message, WiFiClient curr_client)
{
  ...
  String response = curr_client.readStringUntil('\r'); curr_client.readStringUntil('\n');
...
void ESP8266WiFiMesh::attemptScan(String message) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    Serial.print(current_ssid + "\t"); Serial.print(index);Serial.print("\t"); Serial.println(target_chip_id);
